...
include_controls 'microsoft-windows-2012r2-memberserver-stig-baseline' do
  ...
  control 'V-1089' do
    ...
    * This warning banner provides privacy and security notices consistent with applicable federal laws, directives, and other federal guidance for accessing this Government system, which includes (1) this computer network, (2) all computers connected to this network, and (3) all devices and storage media 
    ...
    * By using this system, you understand and consent to the following:
    ...
    desc 'fix', 'Configure the policy value for Computer Configuration >> Windows Settings >> Security Settings >> Local Policies >> Security Options >> 
    ...
    * Personal use of social media and networking sites on this system is limited as to not interfere with official work duties and is subject to monitoring.
    * By using this system, you understand and consent to the following:
    - The Government may monitor, record, and audit your system usage, including usage of personal devices and email systems for official duties or to conduct HHS business. Therefore, you have no reasonable expectation of privacy regarding any communication or data transiting or stored on this system. 
    At any time, and for any lawful Government purpose, the government may monitor, intercept, and search and seize any communication or data transiting or stored on this system.
    - Any communication or data transiting or stored on this system may be disclosed or used for any lawful Government purpose'
  end

  control 'V-1098' do
    title 'The reset period for the account lockout counter must be configured to 120 minutes or greater on Windows 2012.'
    ...
    desc 'fix', 'Configure the policy value for Computer Configuration >> Windows Settings >> Security Settings >> Account Policies >> Account Lockout Policy >> "Reset account lockout counter after" to at 
    least "120" minutes.'
  ...
  control 'V-1099' do
    ...
    Configuring this to "0", requiring an administrator to unlock the account, is not a finding.'
  ...
  control 'V-1107' do
    ...
    desc 'A system is more vulnerable to unauthorized access when system users recycle the same password several times without being required to change to a unique password on a regularly scheduled basis. This 
    enables users to effectively negate the purpose of mandating periodic password changes.  The default value is 12 for Windows domain systems. CMS has decided this is the appropriate value for all Windows systems.'
    desc 'check', 'Verify the effective setting in Local Group Policy Editor.
    Run "gpedit.msc".
  ...
  control 'V-1174' do
    ...
    desc 'fix', 'Configure the policy value for Computer Configuration >> Windows Settings >> Security Settings >> Local Policies >> Security Options >> "Microsoft Network Server: Amount of idle time required before suspending session" to "30" minutes or less.'
  end
  
  control 'V-3373' do
    desc 'Computer account passwords are changed automatically on a regular basis. This setting controls the maximum password age that a machine account may have. This setting must be set to no more than 60 days, ensuring the machine changes its password monthly.'
    desc 'check', 'If the following registry value does not exist or is not configured as specified, this is a finding:
    ...
    desc 'fix', 'Configure the policy value for Computer Configuration -> Windows Settings -> Security Settings -> Local Policies -> Security Options -> "Domain member: Maximum machine account password 
  ...
  control 'V-3472' do
    title 'The time service must synchronize with an appropriate CMS-approved time source.'
    ...
    If systems are configured with a "Type" of "NTP", including standalone systems and the forest root domain controller with the PDC Emulator role, and do not have a CMS-approved time server defined for "NTPServer", this is a finding.

    If an alternate time synchronization tool is used and is not enabled or not configured to synchronize with a CMS-approved time source, this is a finding.
    ...
    Time synchronization will occur through a hierarchy of time servers down to the local level. 
    Clients and lower-level servers will synchronize with an authorized time server in the hierarchy.'
  end
  ...
  control 'V-4108' do
    ...
    Value Type: REG_DWORD
    Value: 80 (or less)'
    desc 'fix', 'Configure the policy value for Computer Configuration -> Windows Settings -> Security Settings -> Local Policies -> Security Options -> "MSS: (WarningLevel) Percentage threshold for the security event log at which the system will generate a warning" to "80" or less.
  ...
  control 'V-4442' do
    ...
    Value Type: REG_SZ
    Value: 0'
    desc 'fix', 'Configure the policy value for Computer Configuration -> Windows Settings -> Security Settings -> Local Policies -> Security Options -> "MSS: (ScreenSaverGracePeriod) The time in seconds before the screen saver grace period expires must be "0".
  ...
  control 'V-14225' do
    title 'Windows 2012/2012 R2 password for the built-in Administrator account must be changed at least every 60 days and when a member of the administrative team leaves the organization.'
    desc 'The longer a password is in use, the greater the opportunity for someone to gain unauthorized knowledge of the password. The password for the built-in Administrator account must be changed at 
    least every 60 days and when any member of the administrative team leaves the organization.
    ...
    If the "PasswordLastSet" date is greater than 60 days old, this is a finding.'
    desc 'fix', 'Change the built-in Administrator account password at least every 60 days and whenever an administrator leaves the organization. More frequent changes are recommended.
  ...
  control 'V-32272' do
    title 'The CMS Interoperability Root CA cross-certificates must be installed into the Untrusted Certificates Store on unclassified systems.'
  ...
  control 'V-36662' do
    title 'Windows 2012/2012 R2 manually managed application account passwords must be changed at least every 60 days and when a system administrator with knowledge of the password leaves the organization.'
  ...
  control 'V-36698' do
    desc 'Allowing biometrics may bypass required authentication methods. Biometrics may only be used as an additional authentication factor where an enhanced strength of identity credential is necessary or 
  ...
  control 'V-36733' do
    desc 'check', 'Determine whether user-level information is backed up in accordance with local recovery time and recovery point objectives. If user-level information is not backed up in accordance with local 
  ...
  control 'V-36734' do
    title 'The operating system must employ automated mechanisms to determine the state of system components with regard to flaw remediation no less often than once every seventy-two (72) hours.'
  ...
  control 'V-57653' do
    title 'Windows 2012 / 2012 R2 must automatically remove or disable temporary user accounts after 30 days.'
    ...
    desc 'check', 'Determine if temporary user accounts are used and identify any that exist.
    If none exist, this is NA.
    ...
    Domain Controllers:
    Enter "Search-ADAccount -AccountExpiring -TimeSpan 30:00:00:00 | FT Name, AccountExpirationDate"
    ...
    If the "WhenCreated" date and "AccountExpirationDate" from the previous query are greater than 30 days apart, this is a finding.
    ...
    If the "Password last set" date and "Account expires" date are greater than 30 days apart, this is a finding. (Net User does not provide an account creation date.)'
  ...
  control 'V-57655' do
    title 'Windows 2012 / 2012 R2 must automatically remove or disable emergency accounts after the crisis is resolved or within 24 hours.'
    ...
    Domain Controllers:
    Enter "Search-ADAccount -AccountExpiring -TimeSpan 1:00:00:00 | FT Name, AccountExpirationDate"
    This will return any accounts configured to expire within the next day. (The "TimeSpan" value to can be changed to find accounts configured to expire at various times such as 30 for the next month.)
    If any accounts identified as emergency administrator accounts are not listed, this is a finding.
    For any emergency administrator accounts returned by the previous query: Enter "Get-ADUser -Identity [Name] -Property WhenCreated" to determine when the account was created.
    If the "WhenCreated" date and "AccountExpirationDate" from the previous query are greater than 1 day apart, this is a finding.

    Member servers and standalone systems:
    Enter "Net User [username]", where [username] is the name of the emergency administrator accounts.
    If "Account expires" has not been defined within 24 hours for any emergency administrator accounts, this is a finding.
    If the "Password last set" date and "Account expires" date are greater than 24 hours apart, this is a finding. (Net User does not provide an account creation date.)'
